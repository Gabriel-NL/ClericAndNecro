    ...
    private void SpawnObject()
{
    ...
    while (attempts < maxAttempts)
    {
        ...
        // Raycast with a layer mask to check only for obstacles (not enemies)
        RaycastHit2D hit = Physics2D.Raycast(transform.position, spawnPosition - transform.position, radius, LayerMask.GetMask("ObstacleLayer"));
